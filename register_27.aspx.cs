...
public partial class register_27 : System.Web.UI.Page
{
    ...
    protected void btnViewRecord_Click(object sender, EventArgs e)
    {
        ...
        if (RegistrationMode == "ofss")
        {
           ...
           <script type='text/javascript'>
               window.onload = function () {{
                ...
                CategoryType == "Private" ? "private" : "regular" 
            );
        ...
        else if (RegistrationMode == "display-registered")
        {
           ...
           <script type='text/javascript'>
               window.onload = function () {{
                   document.getElementById('displayRegistered').checked = true;
                   document.getElementById('{0}').checked = true;
                   handleRegModeChange();
               }};
           </script>",
    ...
    protected void rptStudentList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            if (registrationMode == "ofss" && categoryType == "Regular")
            {
    ...
    protected void btnRegister_Click(object sender, EventArgs e)
    {
        ...
        string url = "studentregform.aspx?studentId=" + Server.UrlEncode(studentId.ToString()) +
                        "&categoryType=" + Server.UrlEncode(categoryType);
        Response.Redirect(url, false);
        // Response.Redirect("register26.aspx?studentData=" + studentId);
        //string encodedStudentId = HttpUtility.UrlEncode(studentId);

        //Response.Redirect("register26.aspx?studentData=" + encodedStudentId);
    }
    ...
    //{
    //    string regMode = Request.Form["regMode"]; 
    //    string regType = Request.Form["regType"];
