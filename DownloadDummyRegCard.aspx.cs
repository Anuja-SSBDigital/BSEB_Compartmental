...
public partial class DownloadDummyRegCard : System.Web.UI.Page
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (fuRowUpload != null && fuRowUpload.HasFile && hfStudentID != null)
            {
                ...
                if (updated)
                {
                    ShowMessage("Success", "File uploaded and saved successfully.", "success");
                    btnGetStudentDummyRegData(null, null);
                }
                else
                {
                    ShowMessage("Database Error", "Failed to update Data.", "error");
                }
            }
            else
            {
                ShowMessage("Warning", "Please select a file to upload.", "warning");
            }
        }
        catch (Exception ex)
        {
    ...
    private void ShowMessage(string title, string message, string icon)
    {
        ...
        string script = string.Format("swal('{0}', '{1}', '{2}');", title, safeMessage, icon);
